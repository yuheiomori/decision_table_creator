...
def export(request):
    ...
    if all([form.is_valid() for form in [condition_formset,
                                         action_formset,
                                         kind_form]]):

        # 全組み合わせ
        if kind_form.cleaned_data["kind"] == 'all':
            variations = list(itertools.product(*[form.variations()
        ...
        # ペア構成
            ...
            variations = list(all_pairs([form.variations()
                                         for form in condition_formset
                                         if form.variations()]))
        ...
        if rule_count > 255:
            messages.error(
                request,
                ('条件の組み合わせが多すぎます。',
                 '全組み合わせを選択している場合はペア構成で試してみてください。'))
            return render_index(action_formset,
                                condition_formset,
                                kind_form,
                                request)
        ...
        response['Content-Disposition'] =\
            'attachment; filename=decision_table.xls'
